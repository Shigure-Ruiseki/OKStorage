...
public class StorageSHRegistry {

    public interface ServerHandler {

        void handle(StorageSH slot, PacketBuffer buf) throws IOException;
    }
    ...
    public static void handleServer(StorageSH slot, int id, PacketBuffer buf) throws IOException {
        ServerHandler handler = SERVER_HANDLERS.get(id);
        if (handler != null) handler.handle(slot, buf);
    }

    public static void handleClient(StorageSH slot, int id, PacketBuffer buf) throws IOException {
